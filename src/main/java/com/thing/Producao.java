...
public class Producao {
    ...
    public String buscaLactacoes() throws SQLException{
        ...
        String sql = "select * from lactacao WHERE data_inicio > '01/01/2014' AND id_animal IN(3484,3495,3497,3498,3499,3501,3502,3503,3504,3507,4191,4214,4215,4217,4219,4225,4235,4256,4257,4321,4330,4332,4336,4338,4339,4340,\n" +
...
"30559,30561,30563,30585,30586,30587,30588,30589,30591,30593,30596,30602,30608,30638,30641,30643,30695,30696,32296,36532)";
    ...
    public String buscaControlesLeiteiros() throws SQLException{
        ...
        String sql = "SELECT DISTINCT animal.id_animal, animal.nome_animal, lactacao.id_lactacao, cl.id_controle_leiteiro\n" +
"                    FROM animal INNER JOIN lactacao on animal.id_animal = lactacao.id_animal\n" +
"				INNER JOIN controle_leiteiro cl on cl.id_lactacao = lactacao.id_lactacao\n" +
"                    WHERE animal.sexo = 'f' AND vivo is not false AND data_inicio > '01/01/2014'\n" +
"				AND cl.percentual_gordura is not null\n" +
"                    ORDER BY animal.id_animal";
        Statement stm = conexao.createStatement();
        rs = stm.executeQuery(sql);
        
        s = String.format("%10s;%20s;%20s\n","id_animal","id_lactacao","id_controle_leiteiro");
        
        while(rs.next()){
            s = s.concat(String.format("%10s;%20s;%20s\n",rs.getString("id_animal"),
